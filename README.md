...
empty, ambiguous, or weak evidence cannot replace valid risk data; when no
authoritative snapshot is available, recommendations apply no risk penalty.
Authoritative risk snapshots are not invalidated merely by elapsed time; run a
new validation and refresh when you want newer designations.
